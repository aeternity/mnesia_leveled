...
%% @doc Verify dirty vs transaction semantics against leveled mnesia backend
%% @author Ulf Wiger <ulf.wiger@feuerlabs.com>

-module(mnesia_leveled_proper_semantics_test).
...
%% against a disc_copies table and a leveled_copies table, after
...
%% Note that this requires the leveled application to be in the path,
...
setup() ->
    {atomic,ok} = mnesia:create_table(d, [{disc_copies, [node()]},
                                          {type, ordered_set},
                                          {record_name, x}]),
    {atomic,ok} = mnesia:create_table(l, [{leveled_copies, [node()]},
                                          {type, ordered_set},
                                          {record_name, x}]),
