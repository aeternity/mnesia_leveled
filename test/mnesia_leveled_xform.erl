...
%% extension machinery
%%
%% Usage: mnesia_ext_leveled_test:recompile(Extension).
%% Usage: mnesia_ext_leveled_test:recompile().
...
create_table(Parameters) when is_list(Parameters) ->
    case lists:keymember(leveled_copies, 1, Parameters) of
        true ->
            %% case lists:member({type, bag}, Parameters) of
            %%     true ->
            %%         ct:comment("ERROR: Contains leveled table with bag"),
            %%         {aborted, {leveled_does_not_support_bag, Parameters}};
            %%     false ->
            ct:comment("INFO: Contains leveled table"),
