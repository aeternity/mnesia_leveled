...
-export([describe_env/0,
         ...
         get_avail_ram/0,
         led_tabs/0, led_tabs/1,
         led_indexes/0, led_indexes/1,
         count_led_tabs/0, count_led_tabs/1,
...
led_tabs() ->
    led_tabs(mnesia_lib:dir()).

led_tabs(Db) ->
    led_tabs(list_dir(Db), Db).
...
led_indexes() ->
    led_indexes(mnesia_lib:dir()).

led_indexes(Db) ->
    led_indexes(list_dir(Db), Db).
...
%% Number of leveled tables + indexes
count_led_tabs() ->
    count_led_tabs(mnesia_lib:dir()).

count_led_tabs(Db) ->
    Fs = list_dir(Db),
    length(led_tabs(Fs, Db)) + length(led_indexes(Fs, Db)).
...
calc_sizes(D) ->
    lists:sort(
      ...
      [{T, dir_size(fname(T, D))} || T <- led_tabs(D)]
      ++ [{I, dir_size(fname(I, D))} || I <- led_indexes(D)]).
