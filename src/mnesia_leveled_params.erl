...
-include("mnesia_leveled_tuning.hrl").
...
load_tuning_parameters() ->
    case application:get_env(mnesia_leveled, tuning_params) of
        {ok, Ps} ->
            case Ps of
                {consult, F} -> consult(F);
                {script, F} -> script(F);
...
store_params(Params) ->
    ...
    NTabs = length(Params),
    Env0= mnesia_leveled_tuning:describe_env(),
    Env = Env0#tuning{n_tabs = NTabs},
    ?dbg("Env = ~p~n", [Env]),
    TotalFiles = lists:sum([mnesia_leveled_tuning:max_files(Sz) ||
                               {_, Sz} <- Params]),
    ...
    lists:foreach(
      fun({Tab, Sz}) when is_atom(Tab);
              ...
              ets:insert(?MODULE, {Tab, led_params(Sz, Env1, Tab)})
      end, Params).

led_params(Sz, Env, _Tab) ->
    MaxFiles = mnesia_leveled_tuning:max_files(Sz) * Env#tuning.files_factor,
    Opts = if Env#tuning.avail_ram > 100 ->  % Gigabytes
                 [{write_buffer_size, mnesia_leveled_tuning:write_buffer(Sz)},
                  {cache_size, mnesia_leveled_tuning:cache(Sz)}];
              true ->
                   []
           end,
